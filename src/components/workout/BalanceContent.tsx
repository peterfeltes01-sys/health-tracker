...
export function BalanceContent() {
  ...
  const { byMuscle, byBucket } = useMemo(() => selectWeeklyVolume(), [recentSessions])
  ...
  return (
    <div className="space-y-5 pb-4">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2">
            {recovery.map((r) => {
              ...
              return (
                <div key={r.muscle} className={`${colors.bg} rounded-2xl px-3 py-2.5 flex items-center gap-2`}>
      ...
      {muscleChartData.length > 0 && (
        <section>
          <h2 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2.5">Wochenvolumen je Muskel</h2>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-3 border border-gray-100 dark:border-gray-800">
            <ResponsiveContainer width="100%" height={Math.max(160, muscleChartData.length * 26)}>
              <BarChart data={muscleChartData} layout="vertical" margin={{ top: 0, right: 8, left: 4, bottom: 0 }}>
                <XAxis type="number" domain={[0, 'dataMax']} tickLine={false} axisLine={false} tick={{ fontSize: 9 }} />
                <YAxis type="category" dataKey="name" width={80} tickLine={false} axisLine={false} tick={{ fontSize: 10 }} />
                <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ fontSize: 11, borderRadius: 8, border: 'none', boxShadow: '0 2px 8px rgba(0,0,0,.1)' }} formatter={(v) => [`${v} Sätze`, '']} />
                ...
                <Bar dataKey="sets" radius={[0, 4, 4, 0]}>
                  {muscleChartData.map((entry) => (
                    <Cell key={entry.name} fill={entry.sets < 10 ? '#94a3b8' : entry.sets > 20 ? '#f87171' : '#6366f1'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      {bucketChartData.length > 0 && (
        <section>
          <h2 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2.5">Drücken / Ziehen / Beine / Core</h2>
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-3 border border-gray-100 dark:border-gray-800">
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={bucketChartData} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
                <XAxis dataKey="name" tickLine={false} axisLine={false} tick={{ fontSize: 10 }} />
                <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 9 }} width={24} />
                <Tooltip cursor={{ fill: 'rgba(99,102,241,.08)' }} contentStyle={{ fontSize: 11, borderRadius: 8, border: 'none', boxShadow: '0 2px 8px rgba(0,0,0,.1)' }} formatter={(v) => [`${v} Sätze`, '']} />
          ...
          <div className="space-y-2">
            {warnings.map((w, i) => (
              ...
              >
                ...
                <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{w.message}</p>
              </div>
            ))}
          </div>
        </section>
